...
@section('title')
    {{ isset($patient) ? 'Aggiorna' : 'Aggiungi' }} Paziente&nbsp;{{ isset($patient) ? $patient->name . ' ' . $patient->surname : '' }}
@endsection

@section('form', isset($patient) ? '/update/patient/' . $patient->id : '/add/patient')

@section('content')
    <x-inputs.img name="img" :init="isset($patient) ? asset($patient->img) : ''" class="absolute w-48 h-48 border-2 rounded-md left-11 top-[15rem] border-primary-clr" >
    ...
    <div class="flex justify-between w-full gap-3">
        <x-inputs.text size="sm" name="moranca" label="Morança *" :value="isset($patient) ? $patient->moranca : ''" />
    ...
    <div class="flex justify-between w-full gap-3">
        ...
        <x-inputs.text size="sm" type="number" name="maxPres" label="Pressione Massima" step="1" :value="isset($patient) ? $patient->max_press : ''" />
        <x-inputs.text size="sm" type="number" name="minPres" label="Pressione Minima" step="1" :value="isset($patient) ? $patient->min_press : ''" />
    </div>
    <div class="flex justify-between w-full gap-3">
        <x-inputs.text size="sm" type="number" name="cardFreq" label="Frequenza Cardiaca" step="1" :value="isset($patient) ? $patient->freq_card : ''" />
